...
int main(void){
  ...
  //TExaS_Init(GRADER, 1384 );         // initialize the Lab 1 grader
  ...
  while(1){
	...
	for(int j=0; j<10; j++){          // runs at about 1000ms
		for(int i=0; i<100; i++){ // runs at about 100ms
			Task0();          // sample microphone
			BSP_Delay1ms(1); //delay for 1ms
		}
